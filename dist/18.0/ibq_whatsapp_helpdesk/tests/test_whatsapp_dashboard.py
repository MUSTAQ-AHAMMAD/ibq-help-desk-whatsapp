...
class TestWhatsappDashboard(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.agent_alice = cls.env["ibq.whatsapp.agent"].create({
            "user_id": cls.alice.id, "role": "agent",
            "status": "available", "max_active_chats": 3,
        })
        cls.agent_bob = cls.env["ibq.whatsapp.agent"].create({
    ...
    def test_routing_picks_the_least_loaded_agent(self):
        ...
        for number in ("+971500000010", "+971500000011"):
            other = self.env["ibq.whatsapp.conversation"]._get_or_create(
                self.account, number
            )
            other.write({"state": "agent", "user_id": self.alice.id})
        self.agent_alice.invalidate_recordset()

        picked = self.env["ibq.whatsapp.agent"]._route(conversation)
    ...
    def test_routing_skips_unavailable_and_full_agents(self):
        ...
        self.env["ibq.whatsapp.conversation"]._get_or_create(
            self.account, "+971500000020"
        ).write({"state": "agent", "user_id": self.alice.id})
        self.agent_alice.invalidate_recordset()

        self.assertFalse(
            self.env["ibq.whatsapp.agent"]._route(conversation),
    ...
    def test_routing_respects_auto_assign_opt_out(self):
        ...
        self.assertEqual(
            self.env["ibq.whatsapp.agent"]._route(conversation), self.agent_bob
        )
    ...
    def test_dashboard_data_on_an_empty_database(self):
        data = self.env["ibq.whatsapp.dashboard"].get_dashboard_data({"period": "7d"})
        ...
        self.assertEqual(len(self.env["ibq.whatsapp.dashboard"].get_bootstrap()["agents"]), 2)
    ...
    def test_volume_series_is_capped_for_long_periods(self):
        data = self.env["ibq.whatsapp.dashboard"].get_dashboard_data({"period": "90d"})
    ...
    def test_queue_scopes(self):
        self._inbound("hi")
        result = self.env["ibq.whatsapp.dashboard"].get_conversations("bot")
        self.assertEqual(len(result["conversations"]), 1)
        self.assertEqual(result["counts"]["bot"], 1)
        self.assertEqual(result["counts"]["waiting"], 0)

        self._inbound("agent")
        result = self.env["ibq.whatsapp.dashboard"].get_conversations("waiting")
        self.assertEqual(len(result["conversations"]), 1)
        self.assertTrue(result["conversations"][0]["needs_reply"])

    def test_queue_search_matches_the_number(self):
        self._inbound("hi")
        hit = self.env["ibq.whatsapp.dashboard"].get_conversations("all", "500000001")
        self.assertEqual(len(hit["conversations"]), 1)
        miss = self.env["ibq.whatsapp.dashboard"].get_conversations("all", "999999")
    ...
    def test_sending_from_the_dashboard_takes_a_bot_chat_over(self):
        ...
        with patch(SEND_PATH, fake_send):
            payload = self.env["ibq.whatsapp.dashboard"].send_message(
    ...
    def test_sending_an_empty_message_is_refused(self):
        ...
        with self.assertRaises(UserError):
            self.env["ibq.whatsapp.dashboard"].send_message(conversation.id, "   ")
    ...
    def test_sending_to_a_closed_chat_is_refused(self):
        ...
        with patch(SEND_PATH, fake_send):
            conversation._close(notify=False)
        with self.assertRaises(UserError):
            self.env["ibq.whatsapp.dashboard"].send_message(conversation.id, "hello?")

    def test_conversation_actions(self):
        conversation = self._inbound("hi").conversation_id
        with patch(SEND_PATH, fake_send):
            self.env["ibq.whatsapp.dashboard"].act_on_conversation(
        ...
        with patch(SEND_PATH, fake_send):
            self.env["ibq.whatsapp.dashboard"].assign_conversation(
        ...
        with patch(SEND_PATH, fake_send):
            self.env["ibq.whatsapp.dashboard"].act_on_conversation(conversation.id, "close")
        conversation.invalidate_recordset()
        self.assertEqual(conversation.state, "closed")
    ...
    def test_unknown_action_is_refused(self):
        conversation = self._inbound("hi").conversation_id
        with self.assertRaises(UserError):
            self.env["ibq.whatsapp.dashboard"].act_on_conversation(conversation.id, "nuke")

    # ------------------------------------------------------------------
    # Roster management
    # ------------------------------------------------------------------
    def test_candidates_exclude_existing_agents(self):
        candidates = self.env["ibq.whatsapp.dashboard"].get_candidate_users()
    ...
    def test_manager_can_add_and_remove_agents(self):
        ...
        agents = self.env["ibq.whatsapp.dashboard"].add_agents([newcomer.id], [self.team.id])
        self.assertEqual(len(agents), 3)

        added = self.env["ibq.whatsapp.agent"].search([("user_id", "=", newcomer.id)])
        self.assertEqual(added.team_ids, self.team)

        agents = self.env["ibq.whatsapp.dashboard"].remove_agent(added.id)
        self.assertEqual(len(agents), 2)

    def test_adding_the_same_user_twice_is_a_no_op(self):
        agents = self.env["ibq.whatsapp.dashboard"].add_agents([self.alice.id])
        self.assertEqual(len(agents), 2)

    def test_non_manager_cannot_change_the_roster(self):
        dashboard = self.env["ibq.whatsapp.dashboard"].with_user(self.alice)
    ...
    def test_set_my_status_requires_a_roster_entry(self):
        ...
        with self.assertRaises(UserError):
            self.env["ibq.whatsapp.dashboard"].with_user(outsider).set_my_status("available")

        payload = self.env["ibq.whatsapp.dashboard"].with_user(self.alice).set_my_status(
            "available"
        )
    ...
    def test_removing_an_agent_with_open_chats_is_refused(self):
        ...
        with self.assertRaises(UserError):
            self.env["ibq.whatsapp.dashboard"].remove_agent(self.agent_alice.id)
