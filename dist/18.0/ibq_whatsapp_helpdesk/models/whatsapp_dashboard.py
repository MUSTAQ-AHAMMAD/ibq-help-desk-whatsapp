...
class WhatsappDashboard(models.TransientModel):
    ...
    def _get_series(self, start, end, scope, msg_scope):
        ...
        # Named "day", not "cursor": Odoo 18's _() inspects caller locals for
        # a variable called "cr" or "cursor" and assumes it is a database
        # cursor, so a date named that crashes every translated string here.
        volume, day = [], start.date()
        while day <= end.date():
            volume.append({
                "date": fields.Date.to_string(day),
                "label": day.strftime("%d %b"),
                "short": day.strftime("%a"),
                "inbound": inbound_map.get(day, 0),
                "outbound": outbound_map.get(day, 0),
            })
